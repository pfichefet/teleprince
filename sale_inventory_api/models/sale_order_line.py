...
class SaleOrderLine(models.Model):
    ...
    def _compute_invoice_date(self):
        ...
        """
        for line in self:
            list_invoice_date = line.invoice_lines.filtered(
                lambda inv_line: inv_line.move_id.state == "posted"
            ).mapped("date")
            list_pos_order_date = line.pos_order_line_ids.order_id.filtered(
                lambda pos_order: pos_order.state in ["paid", "done", "invoiced"]
            ).mapped("date_order")
    ...
    def prepare_bo_report_line(self, report):
        ...
        for stock_move in self.move_ids:
            for sml in stock_move.move_line_ids:
                ...
                move_quantity += sml.product_uom_id._compute_quantity(
                    sml.qty_done, self.product_uom
                )
                sn_values.append(values)
        quantity_left = self.product_uom_qty - move_quantity
        if not float_is_zero(
            quantity_left, precision_rounding=self.product_uom.rounding
        ):
            values = base_values.copy()
            values.update(
                {
