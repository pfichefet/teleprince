...
class BOReportLineAbstract(models.AbstractModel):
    ...
    report_id = fields.Many2one(
        "bo.report", string="B&O Report", readonly=True, ondelete="cascade"
    )
    ...
    lot_id = fields.Many2one(
        "stock.lot", string="Lot", domain="[('product_id', '=', product_id)]"
    )
    company_id = fields.Many2one(
        "res.company", string="Company", related="report_id.company_id"
    )
    ...
    def check_data_validity(self):
        ...
        if not match_product_ref:
            error = True
            error_msg += _(
                "The product identifier must be string that consists of only digits.\n"
        ...
        if not match_qty:
            ...
            if error_msg:
                error_msg += "\n"
            error_msg += _(
                "The quantity must a be a string.\n"
        ...
        store_id = (
            self.warehouse_id.store_identifier or self.company_id.b_and_o_api_store
        )
        ...
        if not match_store_id:
            ...
            if error_msg:
                error_msg += "\n"
            error_msg += (
                _("You must defined a Store identifier on the warehouse %s.")
        ...
        if sn_name:
            ...
            if not match_sn:
                ...
                if error_msg:
                    error_msg += "\n"
                error_msg += _(
                    "The SN must be string that consists of only digits.\n"
    ...
    def prepare_api_json_values(self):
        ...
        if self.product_id:
            values = {
                "storeId": (
                    str(self.warehouse_id.store_identifier)
                    if self.warehouse_id and self.warehouse_id.store_identifier
                    else str(self.company_id.b_and_o_api_store)
                ),
