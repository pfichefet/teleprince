                ...
                if (
                    ...
                    for line in all_editable_lines:
                        ...
                        if product.service_type == "manual":
                            line.with_context(
                                fsm_no_message_post=True
                            ).qty_delivered = new_line_qty
                        line.with_context(fsm_no_message_post=True).product_uom_qty = (
                            new_line_qty
                        )
                        if task.under_warranty:
                            line.price_unit = 0
                    ...
                    vals = {
                        ...
                        # Custom code Start
                        "route_id": (
                            warehouse.fsm_route_id.id
                            if warehouse.fsm_route_id
                            else False
                        ),
                        # Custom code End
                    }
                    ...
                    if task.sale_order_id.order_line:
                        vals["sequence"] = (
                            max(task.sale_order_id.order_line.mapped("sequence")) + 1
                        )
